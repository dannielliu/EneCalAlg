...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  double beamlow = beamene - 0.3;
  double beamup  = beamene + 0.3;
  
  char fname[1000];
  sprintf(fname,"%s/plot_mumu_cms.root",outputdir.c_str());
...
void MUMU::FitSpe(std::vector<APair> &evts, double beame,  char* namesfx)
{
  ...
  //~~~~~~~~~~part start~~~~~~~~
    ...
    sprintf(tmpchr,"factor_%.4f",1.0);
    double peakt,errt;
    MUMU::FitSpectrum(dataraw,peakv,tmpchr,peakt,errt);
    ofstream outf("peakdismumu",std::ios::app);
    outf << beame << '\t' << /* namesfx<<"\t"<< */ peakt << "\t" << errt <<std::endl;
    
    double factor = 1.000188;
    dataraw->Reset();
    for (int evtid=0; evtid<evts.size();evtid++){
	  APair evt = evts.at(evtid);
...
void MUMU::FitSpectrum(TTree *&dataraw,double beame, char* namesfx, double &peak, double &peakerror)
{
   ...
   RooRealVar sigma("sigma","width of gaussian",0.014,0.01,0.04);
   ...
   RooRealVar background("background"," ",240,0,1000);
 ...
 //RooPolynomial ground("ground","ground",x,RooArgList(a0,a1));
   ...
   RooRealVar alpha1("alpha1","#alpha",3.5,1,10);
   RooRealVar nnn1("n1","n",100,0,300);
...
// xframe->Draw();
  ...
  gaus.fitTo(*dataset,Range(peakvalue - 0.03, peakvalue + 0.03));
  dataset->plotOn(xframe);
  gaus.plotOn(xframe);
  xframe->Draw();
...
//pt->AddText(tmpchr);
  sprintf(tmpchr,"#chi^{2}/(%d-%d) = %5.6f",nBins,3,xframe->chiSquare(Npar));
  pt->AddText(tmpchr);
  pt->Draw();
