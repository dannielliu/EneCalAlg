...
// double factorstart=0.995;
   ...
   purepar.open("par");
   char fname[100];
   sprintf(fname,"%s/plot_kstar_AR.root",outputdir.c_str());
   ...
   TH1D *hmD0 = new TH1D("hmD0","M_{K #pi}",200,0,2);
 ...
 //}
   ...
   double m0=peakvalue;
   double sigma_m = 0.0474;//0.0024 for phi,
...
//////// evt.set(kam,pip);
   ...
   ftmp->WriteTObject(hpka,"hpka_K2Kpi");
   ftmp->WriteTObject(hppi,"hppi_K2Kpi");
   ftmp->WriteTObject(hmD0,"hmD0_K2Kpi");
   ftmp->Close();
   delete ftmp;
   f->cd();
   return ;
...
{
  double peak = 0.89594;
  double beamlow=peak-0.7;
  double beamup=peak+0.7;
  // for factor fit
...
void KPI::FitSpectrum(TTree *&dataraw, double beame, char* namesfx, double &peak, double &peakerror)
{
   ...
   int Npar;
   double peakvalue = 0.89581;
   double beamlow = 0.7;
   double beamup  = 1.1;
   // try to use roofit
   RooRealVar x("x","energy",peakvalue,beamlow,beamup,"GeV");
   RooRealVar mean("mean","mean of gaussian",peakvalue,beamlow,beamup);
   RooRealVar sigma("sigma","width of gaussian",0.020,0.0010,0.03);
   RooRealVar sigma2("sigma2","width of gaussian",0.075,0.03,0.10);
   ...
   RooRealVar width("width","BW width",0.0474,0.0474,0.0474);
   RooVoigtian sig("sig","signal",x,mean,width,sigma);
