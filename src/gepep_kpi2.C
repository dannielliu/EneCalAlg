...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   ofpar<<"k- pi+ algrithm #2: will give factors for pion"<<std::endl;
   ...
   std::string fitepsname  = outputdir+"/fitkpi2.eps";
   ...
   for (int i=0;i<pointNo;i++){
      ...
      for (Long64_t jentry=0; jentry<nentries;jentry++) {
		 ...
		 if(cos(delthe[besidx])>-0.9) continue;
	  ...
	  char tmpchr[100];
	  sprintf(tmpchr,"data_kpi2_%02d",fittimes);
      data_kpi = new RooDataHist(tmpchr,"data_kpi2",x,h1);
...
#ifdef gepep_kpi2_cxx
gepep_kpi2::gepep_kpi2(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("RValue_kpi2_3850.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("RValue_kpi2_3850.root");
      }
...
gepep_kpi2::~gepep_kpi2()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t gepep_kpi2::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t gepep_kpi2::LoadTree(Long64_t entry)
{
...
void gepep_kpi2::Init(TTree *tree)
{
...
void gepep_kpi2::Show(Long64_t entry)
{
...
Int_t gepep_kpi2::Cut(Long64_t entry)
{
